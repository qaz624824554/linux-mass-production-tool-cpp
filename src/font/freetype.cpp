...
Freetype::Freetype(Font *font_ptr) {
  this->name = "freetype";

  this->next = font_ptr->font_opr_ptr;
  font_ptr->font_opr_ptr = this;
}

void Freetype::font_init(char *filepathname) {
  int error = FT_Init_FreeType(&(this->library));
  std::cout << "freetype初始化成功1" << std::endl;

  error = FT_New_Face(this->library, filepathname, 0, &(this->face));

  std::cout << error << std::endl;
  std::cout << "freetype初始化成功2" << std::endl; // 无法打印，需要测试Freetype能否编译通过

  if (error) {
    printf("FT_New_Face error\n");
    throw "FT_New_Face error\n";
  }

  std::cout << "freetype初始化成功3" << std::endl;
}

void Freetype::set_font_size(int size) { FT_Set_Pixel_Sizes(face, size, 0); }

int Freetype::get_font_bitmap(unsigned int char_code,
...
int Freetype::get_string_region_car(char *str,
